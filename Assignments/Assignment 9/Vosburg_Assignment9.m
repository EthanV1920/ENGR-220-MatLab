...
while (true)
    ...
    while (firstBank < 1000000)
        ...
        firstBank = firstBank * 1.0055;
        firstBankTime = firstBankTime + 1;
    end

    while (secondBank < 1000000)
        secondBank = secondBank + 30;
        secondBank = secondBank * 1.0045;
        secondBankTime = secondBankTime + 1;
    end

    if (firstBankTime < secondBankTime)
        fprintf('It will take %d years to reach $1,000,000 in First National Bank.\n', firstBankTime);
        fprintf('It will take %d years to reach $1,000,000 in Second National Bank.\n', secondBankTime);
        break;
    end
...
figure(50);
problem5bPlot = axes;
fplot(@(t) Height(t), [0, 30], 'b-', 'Parent', problem5bPlot);
ylabel(problem5bPlot, 'Height (m)');
xlabel(problem5bPlot, 'Time (s)');
title(problem5bPlot, 'Height vs. Time');
...
heightArray = [[0:0.5:30]' Height([0:0.5:30])'];
[value index] = max(heightArray);
maxHeight = heightArray(index(2), 1);

fprintf('\nThe rocket starts to fall back to the ground at %.2f seconds.\n', maxHeight);
